...
| **SM-06** | Claude calls `run_document` through our tool loop and the data comes back into the conversation | `ANTHROPIC_API_KEY`, `GRAPH_API_KEY` | ⏸ **written, never run** — no longer blocked |
...
| **SM-09** | ⚠️ **MANUAL — there is no script.** A browser wallet (OKX, MetaMask as fallback) adds Arc and signs a transaction. Walk `09-browser-stake.md` by hand | a browser with OKX or MetaMask; nothing from `.env` | ⏸ **checklist written, not walked** |
