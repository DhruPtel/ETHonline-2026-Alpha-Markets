...
5. ⚠️ **This project is on Vercel's HOBBY plan, so a function is capped at 60 seconds — not 300.**
   Confirmed 2026-09-09 from the Vercel API (`billing.plan = hobby`), not inferred from the dashboard.
   **A declared `maxDuration` above the cap is silently clamped**: Vercel accepts it, the build emits
   no warning, and the deployment goes READY carrying 60. Four console routes declare 300 and none of
   them has it. ⚠️ **This is the fifth thing in this project that looked live and was not** — after
   `extensionAlias` accepted-and-ignored, the gateway error strings that never existed, the env var
   set to empty, and a research note's summary cell.
   ⚠️ **A route that declares nothing gets the platform default, roughly 10 seconds.**
   `app/api/reports/[hash]` — the route where money moves — had no declaration until 2026-09-09
   against a measured 8.6-second settle, a 1.4-second margin. It now declares **60** and explains why
   at the site. Decision 2 below carries the measured generation figures.
...
request-driven advance, no `workflow/`**. ⚠️ **Commissioning through the app is Phase 4 or later.**
What we give up: a visitor cannot ask for a new report. What we get: the phase does not spend three
units on a scheduler before the thing being scheduled exists.

⚠️ **The reasoning was corrected on 2026-09-09. The decision stands; the reason it was written down
does not.** It used to read *"those exist because a 300-second Vercel invocation cannot hold a
multi-deployment report generation, and in Phase 3 nothing asks it to"* — which implied the 300
seconds was the headroom that made a CLI-only design comfortable.

**There is no 300-second invocation on this project.** It is on Vercel's **Hobby** plan — confirmed
from the API, `billing.plan = hobby`, not inferred — where a function is capped at **60 seconds** and
a declared `maxDuration = 300` is *silently clamped*. Four console routes declare 300 and every one
of them gets 60.

The measured figures, so anyone revisiting this works from real numbers rather than the old one:

| | |
|---|---|
| Hobby function ceiling | **60 s** — the real one |
| `maxDuration` declared on four console routes | 300 s — accepted, clamped, no warning |
| `execute`'s `DEFAULT_BUDGET.maxWallClockMs` | 240,000 ms — **4× beyond the ceiling; it can never fire in the app** |
| routine generation, measured twice | **34.0 s** (compose 7.7 · execute 0.65 · narrate 25.9) and **46.7 s** (compose 17.2 · execute 2.4 · narrate 27.1) |
| …as a share of the real ceiling | **57% and 78%** |

⚠️ **The two model calls are ~98% of a run and neither carries a budget, a wall clock or a timeout.**
`execute`'s budget guards the one stage that finishes in seconds.

**The decision is still right, for a better reason: generation is a CLI job, where no platform ceiling
applies at all.** `scripts/ops/report.ts` runs in a local process, so a wide directive there simply
takes as long as it takes and the 240,000 ms budget can genuinely fire and report `status: 'budget'`.
The only in-app generator is `app/api/console/generate`, which is **throwaway and deleted before
submission**. So nothing that ships depends on an invocation being long enough — which is what makes
the absence of job machinery safe, and it would have been the honest reason to give in the first place.

⚠️ **What would overturn it:** commissioning a report from the product, which decision 2 defers to
Phase 4 or later. On a 60-second ceiling that is not a matter of trimming a directive — a routine run
already uses 78% of it — so it would need either the job-progression apparatus this decision removed,
or a plan upgrade. **Do not schedule app-side generation on this plan without one of the two.**
