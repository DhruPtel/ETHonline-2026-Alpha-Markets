...
//   4. the Arc committer is ours        `arc.ts analystIdentity()`
//
// ⚠️ **Nothing here spends.** Two Mirror GETs, one Circle wallet lookup, one database read. If this
// file ever finds itself sending a transaction, something is wrong.
//
...
// Phase 3's Unit 10 moved them on purpose, because H2.4 asks for a lifecycle operation on camera —
// **so the act that satisfies H2.4 would break an admission check built on holding.** Who *issued* a
// token is permanent and sits in the deploy transaction forever; who *holds* it is a lifecycle fact
// that is supposed to change. This file reads the deploy transaction and never asks `balanceOf`.
//
...
// can fix it. Its value is narrower: it names **exactly what to re-check**, and every one of those is
// independently readable off two public chains by somebody who believes nothing we say. Detectable
// by anyone, enforced by no one.
 ...
 *
 * There is no Arc transaction here on purpose — the commit has not happened when this is built,
 * which is the point of a check that runs *before* money moves. `recordArcTransaction` completes the
 * row once the commit has a hash.
 */
export interface BindingEvidence {
  ...
  /** Arc: the address that will commit. Check 4 — the identity guard already refused a mismatch. */
...
/**
 * Run the checks. Returns what it saw, or throws `BindingRefused` saying which one failed.
 *
...
export async function checkBinding(reportHash: string): Promise<BindingEvidence> {
  ...
  // ── 4 · who we would be committing as. `analystIdentity()` throws on a mismatch of its own. ──
  ...
  // log is the ONLY place the report hash survives on Hedera. `ats.ts tokenize()` checks it at issue
  // time; this re-checks it at commit time, against the chain rather than against our row.
  ...
  //
  // ⚠️ Checking both is strictly stronger than checking either: two independent encodings of one
  // identity, from two different parts of the same record, and `config/analysts.ts` already stores
  // both forms on purpose ("deriving one from the other at each call site is where they drift"). A
  // mismatch in either is a refusal.
 ...
 *
 * ⚠️ **`raw` is canonical JSON as TEXT and never jsonb** — the rule `settlement_evidence` holds, for
 * the reason given at `settle.ts recordSettlement`: jsonb would not hand back the bytes that were
 * written.
 *
 * ⚠️ **`arc_tx` is NULL here and that is correct.** The check runs before the commit, so there is no
 * Arc transaction yet. `market.ts commit()` calls `recordArcTransaction` once it has a hash, and until
 * then a null means "admitted, not yet committed" rather than anything missing.
 *
...
/** The fifth identifier, once it exists. ⚠️ `market.ts commit()` calls it after the commit lands. */
