// POST /api/markets/[id]/refresh — take a stake transaction hash, confirm it, record it.
//
...
export async function POST(
  ...
  const [market] = await db()<{ id: string }[]>`
    SELECT id FROM markets WHERE chain_market_id = ${id} AND contract_address = ${contractAddress}`;
  ...
  const staked = receipt.logs
    ...
    .find((parsed) => parsed?.name === 'Staked' && (parsed.args[0] as bigint).toString() === id);
  if (!staked) {
    return NextResponse.json(
      { recorded: false, error: `${txHash} mined but carries no Staked event for market ${id}`, ...(await pools()) },
      { status: 422 },
    );
  }
