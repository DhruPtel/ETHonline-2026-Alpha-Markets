// Is this market a forecast, or was its answer already knowable? ⚠️ **Each rule written once.**
//
...
// on the string is filtering on a habit.
//
// ── ⚠️ WHY THIS IS A TypeScript PREDICATE AND NOT A SQL FRAGMENT ─────────────────────────────────
//
// The rule had grown several spellings — `(observation_end <= created_at)` in SQL on pages, and a
// `Date`-comparing arrow function in `scripts/ops/score.ts`. The obvious lift is to export the SQL
// expression as a string and interpolate it, and that was tried and rejected: a raw fragment has to
// go through `sql.unsafe` to reach a parameterised query, which is a footgun this codebase has been
// careful to avoid everywhere else, and it would still leave the TypeScript callers with a second
// spelling. **So the rule is a predicate over the two timestamps, and SQL callers select the two
// columns and apply it.**
//
// ⚠️ Find the callers by grepping for the comparison, not from a list: during the consolidation **the
// list of known SQL copies said two and the repository said three.**
//
...
//
// ⚠️ **`agent/context.ts` had NO rehearsal filter at all**, which PHASE-7 §4.3 requires. It now
...
// ── ⚠️ THE THIRD CATEGORY: PAST-POSTED, AND IT IS NOT A FLAG ─────────────────────────────────────
//
...
// arithmetic says *forecast* and lets it straight into the record. PHASE-8 §2.2 calls it the single
// easiest thing in the phase to get wrong, because it fails silently into an inflated record rather
// than into an error.
//
...
// `markets.observed_day`. It cannot be set by a naming convention or turned off by a flag on a call
// site. `spec.ts demoQuestionCore()` is the only builder that produces one and it refuses anything
// else; this is the read-side half of that pair.
//
// ── ⚠️ WHY THIS MUST NOT BE INLINED AS SQL EITHER ───────────────────────────────────────────────
//
// **`pastPosted` is a worse candidate for inlining than the rehearsal rule, not a better one**, and it
// had six call sites within a day of being written.
//
...
// flip category. `dayStart()` is explicitly `Date.UTC`. The rehearsal spellings at least agreed with
// each other; these two would not.
//
...
// date and throws a sentence naming it; a Postgres cast raises a query error that takes down **every
// row on the page**, in a query nobody looking at the page would think to suspect.
